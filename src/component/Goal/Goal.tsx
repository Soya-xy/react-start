...
const Index = (_props: any, ref: any) => {
  ...
  const columns = [
...

    {
      ...
      render: (id: number, item: any) => {
        return <div>{item?.GoalName?.name}</div>
      }
    }
    ,
    {
      ...
      align: 'center',
      render: (id: number, item: any) => {
        return <div>{item?.GoalName?.Department2}</div>
      }
    }
    ,
    {
  ...
  const del = (data: any) => {
    modal.confirm({
      ...
      maskClosable: true,
      onOk: () => {
        req.post('Goal/delGoal', { id: data.id }).then(res => {
          if (res.code == 1) {
            refresh()
          } else {
  ...
  return (
    <React.Fragment>
      ...
      <CustomModal
        ...
        width={360}
        onCancel={onCancel}
        title={(<Title title={`${type === 'edit' ? '编辑' : '添加'}业绩目标配置`} />)}
      >
