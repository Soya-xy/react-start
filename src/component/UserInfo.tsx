...
import FileList, { GetItems } from '../common/FileList';
...
// 上传图片组件
export const CustomUpload: React.FC<types> = ({ value = '', className = '', type = 1, isAvatar = true, title = '修改头像', onChange }) => {
                ...
                {value &&
                    (isAvatar ?
                        ...
                        (<div className='flex flex-col items-center h-full'>
                            ...
                            /> : <GetItems item={{ type, url: value }} className='wh-full' />}
                            <Button
                                className="mt-2"
                                onClick={(e) => {
                                    e.stopPropagation();
    ...
    useEffect(() => {
        req.post('AdminInfo/info').then(res => {
            ...
            if (res.code == 1) {
                ...
                form.setFieldsValue({
                    ...res.data,
                    entry_time: dayjs(res.data.entry_time).isValid() ? dayjs(res.data.entry_time) : dayjs(),
                    birth: dayjs(res.data.birth).isValid() ? dayjs(res.data.birth) : dayjs(),
                    employment_time: dayjs(res.data.employment_time).isValid() ? dayjs(res.data.employment_time) : dayjs(),
                    ...
                    <div className='flex w-full'>
                        ...
                        <div className='ml-4 flex-1'>
                            ...
                            <div className='flex gap-4'>
                                ...
                                {/* 一级部门 */}
                                <Form.Item className='item32' name="department" label="部门">
                                    <Input disabled />
                                </Form.Item>
                            </div>
                            <div className="flex gap-4">
